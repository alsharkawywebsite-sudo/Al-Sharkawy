...
import { b as House, c as Settings, f as Percent, g as LogOut, h as Menu, i as Tags, n as Utensils, t as X, y as LayoutDashboard } from "../_libs/lucide-react.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/admin-D9HoJFNR.js
...
function AdminLayout() {
	...
	const [isAuthChecked, setIsAuthChecked] = (0, import_react.useState)(false);
	(0, import_react.useEffect)(() => {
		let mounted = true;
		supabase.auth.getUser().then(({ data: { user } }) => {
			if (!mounted) return;
			if (isLoginPage && user) router.navigate({
				to: "/admin",
				replace: true
			});
			else if (!isLoginPage && !user) router.navigate({
				to: "/admin/login",
				replace: true
			});
	...
	if (!isAuthChecked) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: "flex min-h-screen items-center justify-center bg-alabaster",
